#!/bin/bash
# make-gotek-images-1.sh
#
# Creates GoTek-compatible .img files from a local mirror of Slackware 1.1.2.
#
...
#
# Unlike 2.0.0, there is no separate rootdisk directory. Boot images come from
# bootdisk/1_44meg/ and all package series dirs live directly in the tree root.
#
...
#
# Usage: bash make-gotek-images-1.sh <slackware-dir> [--force]
#   <slackware-dir>  path to the slackware version directory (e.g. slackware-1.1.2)
...
if [ -z "$TARGET_DIR" ]; then
    ...
    echo "  e.g. $(basename "$0") slackware-1.1.2"
    exit 1
fi
...
BOOT_DIR="$TARGET_DIR/bootdisk/1_44meg"  # gzipped raw kernel images
OUT_DIR="$SCRIPT_DIR/gotek/$VERSION"     # all generated .img files go here

# Number of disks in each package series.
# These match the disk counts used by the original Slackware 1.1.2 disk set.
...
SERIES_ORDER="a ap d e f i iv n oi oop t tcl u x xap xd xv y"
...
#
# bootdisk/1_44meg/ contains gzipped raw kernel images. Each is decompressed
# and written sector-by-sector directly into the image file. If the kernel
# image is smaller than 1.44MB it is zero-padded to the full floppy size so
# the GoTek drive sees a complete disk.
##############################################################################
...
#
# Unlike 2.0.0, series dirs live directly in the tree root rather than under
# a slakware/ subdirectory.
##############################################################################
